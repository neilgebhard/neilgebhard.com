...
With OAuth, we'll be able to leverage the authorization workflows of major web services (Google, GitHub, Facebook, etc.) to authenticate users. It will supply us with security (no credentials are provided) and robustness (e.g. password management is done by the OAuth provider). Let's start.
...
You'll probably need to hold some set of data about your users. Add the columns you'd like to hold about them here. Go to the Table Editor tab and click on Create a new table.

Supabase uses PostgreSQL, so you may need some knowledge about managing relational database systems. I'll be adding 3 columns to mine: id (uuid), email (text), and username (text).
...
You'll need to register your application with your select OAuth provider(s) in order to use it. They'll need certain information like your homepage URL and callback URL. For the homepage URL, use http://localhost:3000 _in development_ and your deployment URL in production. The callback URL should be your **Supabase API URL** with the `/auth/v1/callback` path.
...
When a user signs up with OAuth, a user row is added to the `auth.users` table. The columns of this table are not modifiable. Also, this table is not publicly accessible via API. To circumvent this, we can use a trigger that activates on insert to the `auth.users` table. This trigger will add a new user to the `public.user` table.
...
Next, create two environment variables to store information about your Supabase project inside your `.env.local` file. These can be found in the _API_ tab in your _Settings_ page.
...
Let's create a login page to log in to our app. I'm using Github as the OAuth provider, but you can just as easily add or use any other OAuth provider to handle your login. You can find more information using OAuth providers in Supabase [here](https://supabase.com/docs/guides/auth).
